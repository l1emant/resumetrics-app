...
import { Loader2, FileText, X, Sparkles, AlertCircle, Briefcase, FileUp } from 'lucide-react';
...
export function ResumeUploader({ onAnalysisComplete, step, setStep }: Props) {
  ...
  useEffect(() => { autoResize(); }, [jd, autoResize]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!file || file.size === 0) { setError('Attach a resume first.'); return; }
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full flex flex-col gap-3">
      ...
      {/* Resume Upload Area */}
      <input type="file" name="resume" ref={fileInputRef} className="hidden" onChange={handleFileChange} accept=".pdf,.doc,.docx" />
      ...
      {!fileName ? (
        <button
          ...
          className="group w-full flex flex-col items-center justify-center gap-2 py-7 rounded-xl border border-dashed border-zinc-700/40 bg-zinc-900/20 hover:border-emerald-600/30 hover:bg-emerald-950/5 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
        >
          <div className="w-10 h-10 rounded-full bg-zinc-800/50 group-hover:bg-emerald-900/20 flex items-center justify-center transition-colors">
            <FileUp className="w-4.5 h-4.5 text-zinc-500 group-hover:text-emerald-400 transition-colors" />
          </div>
          <div className="text-center">
            <p className="text-[13px] text-zinc-400 group-hover:text-zinc-300 transition-colors">Click to upload resume</p>
        ...
        <div className="w-full flex items-center justify-between px-4 py-3 rounded-xl border border-emerald-800/25 bg-emerald-950/8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[13px] text-zinc-200 max-w-52 truncate">{fileName}</p>
              <p className="text-[11px] text-emerald-500/60">Ready for analysis</p>
            </div>
          </div>
          {!isProcessing && (
            <button
      ...
      {/* Job Description */}
      <div className="relative rounded-xl border border-zinc-800/40 bg-zinc-900/20 focus-within:border-zinc-700/50 transition-colors overflow-hidden">
        <div className="flex items-center gap-2 px-3.5 pt-2.5 pb-0">
          <Briefcase className="w-3 h-3 text-zinc-600" />
          <span className="text-[10px] text-zinc-600 uppercase tracking-wider">Job Description</span>
        ...
        <textarea
          ...
          style={{ minHeight: '72px', maxHeight: '240px', scrollbarWidth: 'thin', scrollbarColor: '#2a2a2a transparent' }}
        />
      </div>
      ...
      {isProcessing ? (
        <div className="flex items-center justify-center gap-2.5 py-3 rounded-xl bg-emerald-950/15 border border-emerald-800/15">
          ...
          <span className="text-xs text-emerald-400/80">{STEPS[step]?.text}</span>
        </div>
      ) : (
        <button
          type="submit"
          ...
          className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium transition-all ${
            !fileName
              ...
              : 'bg-emerald-600 hover:bg-emerald-500 text-white cursor-pointer active:scale-[0.98] shadow-lg shadow-emerald-900/20'
          }`}
        >
